...
class SMA_1(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.buyprice = None
        self.buycomm = None
        self.sma_fast = bt.indicators.SimpleMovingAverage(self.dataclose, period=20)
        self.sma_mid = bt.indicators.SimpleMovingAverage(self.dataclose, period=50)
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.sma_fast[0] > self.sma_mid[0]:
                ...
                # BUY, BUY, BUY!!! (with all possible default parameters)
                # self.log('BUY CREATE, %.2f' % self.dataclose[0])
                self.log('BUY CREATE, %.2f, SMA Short %.2f, SMA Mid %.2f' %
                         (self.dataclose[0],
                          self.sma_fast[0],
                          self.sma_mid[0]))
            ...
            if self.sma_fast[0] < self.sma_mid[0]:
                # SELL, SELL, SELL!!! (with all possible default parameters)
                # self.log('SELL CREATE, %.2f' % self.dataclose[0])
                self.log('SELL CREATE, %.2f, SMA Short %.2f, SMA Mid %.2f' %
                         (self.dataclose[0],
                          self.sma_fast[0],
                          self.sma_mid[0]))
