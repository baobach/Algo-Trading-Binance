...
class BinanceAPI:
    ...
    def download_1m(self, symbol, begin = '1 Oct 2023', end = None):
    ...
    def download_15m(self, symbol, begin = '1 Oct 2023', end = None):
        """Download 1 Minute trading data from Binance.
        ...
        # Download 1_Min trading data
        klines = self.bclient.get_historical_klines(symbol, Client.KLINE_INTERVAL_15MINUTE, start_date.strftime("%d %b %Y %H:%M:%S"), end_date.strftime("%d %b %Y %H:%M:%S"), 1000)
        data = pd.DataFrame(klines, columns = ['timestamp', 'open', 'high', 'low', 'close', 'volume', 'close_time', 'quote_av', 'trades', 'tb_base_av', 'tb_quote_av', 'ignore' ])
        data['timestamp'] = pd.to_datetime(data['timestamp'], unit='ms')
        data.set_index('timestamp', inplace=True)
        ...
        data = data.reindex(columns=['open', 'high', 'low', 'close', 'adj_close', 'volume'])

        print('Completed!')

        return data
        
    def download_1hour(self, symbol, begin = '1 Oct 2023', end = None):
            """Download 1 Minute trading data from Binance.
            ...
            # Download 1_Min trading data
            klines = self.bclient.get_historical_klines(symbol, Client.KLINE_INTERVAL_1HOUR, start_date.strftime("%d %b %Y %H:%M:%S"), end_date.strftime("%d %b %Y %H:%M:%S"), 1000)
            data = pd.DataFrame(klines, columns = ['timestamp', 'open', 'high', 'low', 'close', 'volume', 'close_time', 'quote_av', 'trades', 'tb_base_av', 'tb_quote_av', 'ignore' ])
            data['timestamp'] = pd.to_datetime(data['timestamp'], unit='ms')
            data.set_index('timestamp', inplace=True)
